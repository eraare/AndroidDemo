...
public class MainFragment1 extends Fragment {
    ...
    private void initValues() {
        ...
        isLighting = sp.getBoolean(Constant.KEY_DEVICE_SWITCH, true);
        currentColor = sp.getInt(Constant.KEY_DEVICE_COLOR, Color.WHITE);
        objectTime = sp.getFloat(Constant.KEY_TIMER, 0);
    }

    private void saveValues() {
        SharedPreferences sp = PreferenceManager.getDefaultSharedPreferences(getContext());
        SharedPreferences.Editor editor = sp.edit();
        editor.putInt(Constant.KEY_DEVICE_COLOR, currentColor).apply();
        editor.putBoolean(Constant.KEY_DEVICE_SWITCH, isLighting).apply();
        editor.putFloat(Constant.KEY_TIMER, objectTime).apply();
    }
...
//                    showTimerDialog();
                    ToastUtill.showToast(mContext, "定时功能正在完善中...", Constant.TOASTLENGTH).show();
                }
                break;
                default:
                    break;
...
//                changeTheBrightness(progress);
    ...
    private BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (TextUtils.equals(action, Constant.ACTION_INIT_STATUS)) {
                initValues();
            }
        }
    };
    ...
    private void registerReceiver() {
        ...
        mFilter.addAction(Constant.ACTION_INIT_STATUS);
